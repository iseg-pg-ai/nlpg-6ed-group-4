...
- Para garantir uma avaliação rigorosa e imparcial do Ministral e do Qwen, implementámos uma arquitetura personalizada Two-Pass Batched. Isto permite-nos contornar as limitações de VRAM do hardware local e alojar um modelo mais capaz, o deepseek-r1-0528-qwen3-8b, como avaliador imparcial através da framework DeepEval. 
...
- Para garantir uma avaliação imparcial e evitar o viés interno de um dos modelos que estamos a testar, utilizámos o padrão LLM-as-a-judge com um terceiro modelo independente. 
- Devido a limitações estritas de hardware local (Assumido 2GB VRAM), escolhemos o llama-3.2-1b-instruct. Como os modelos mais pequenos frequentemente têm dificuldade em seguir instruções estritas e tendem a gerar texto conversacional, intercetámos os modelos Pydantic dinâmicos do DeepEval e traduzimo-los para esquemas JSON da OpenAI para forçar outputs válidos.

**Sobre a mudança do judge model de Llamma3.2-1B para deepseek-r1-0528-qwen3-8b**
- As boas práticas de LLMOps defendem que o juiz deve ser **pelo menos tão capaz quanto o modelo avaliado** (idealmente superior). Como o llama-3.2-1b era mais pequeno que os nossos geradores (Ministral 3B e Qwen 3.5 2B), as suas pontuações de Faithfulness e Relevancy eram ruidosas: penalizava respostas corretas e tinha dificuldade em distinguir a qualidade entre os dois modelos.
- Por isso, substituímo-lo pelo **deepseek-r1-0528-qwen3-8b**: um *distill* de 8B do DeepSeek-R1 com capacidades de raciocínio, muito mais fiel na verificação de factos contra o contexto recuperado. Por ser um modelo de raciocínio, aumentámos a janela de contexto para **8192 tokens** (`context_length=8192`) e o limite de geração para **4096 tokens**, para acomodar as cadeias de pensamento (*reasoning tokens*) que produz antes de pontuar, bem como os *timeouts* das chamadas (10s → 600s) para a maior latência de inferência.
...
- Numa primeira fase, recuperamos um número alargado de documentos (500 chunks via `RETRIEVAL_TOP_K`). De seguida, utilizamos um modelo LLM focado em re-ranking para reavaliar e reordenar os chunks (mencionado na **Apresentação 5, Slide 21**), mantendo apenas os 10 mais relevantes via `RERANK_TOP_K`. Isto garante que o LLM final recebe apenas os chunks com maior densidade de informação relevante, mitigando a "diluição de atenção" e aumentando drasticamente a precisão da resposta.
- O `retriever` distingue os dois valores de `top_k`: o número de chunks obtido da base de dados e o número devolvido após o re-ranking. Se nenhum modelo de re-ranking estiver configurado, os resultados da pesquisa híbrida são apenas truncados para o limite de `RERANK_TOP_K`.
